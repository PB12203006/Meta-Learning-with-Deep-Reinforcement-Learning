...
def create_Q_network(LSTM_HIDDEN, METADATA_DIM, ACTION_DIM, PERFORMANCE_DIM, CHOICE_DIM, DENSE_UNITS):
    ...
    qsa_model.save('models_One/model_0')
    return
...
def train_network(iteration_idx, meta_data, model_choices, model_performances, actions, values):
    ...
    from keras.layers import Input, Dense, LSTM, Concatenate
    from keras import backend as K
    qsa_model = models.load_model('models_One/model_%d' % iteration_idx)
    ...
    qsa_model.save('models_One/model_' + str(iteration_idx + 1))
    return
...
def evaluate(iteration_idx,  num_threads, sample_count):
    ...
    np.save('models_One/iteration_' + str(iteration_idx) + '_performance', averaged_curve)
    np.save('models_One/iteration_' + str(iteration_idx) + '_variance', curve_std)
...
def thread_evaluate(lock, thread_idx, return_dict, iteration_idx, EPSILON, data_needed):
    from keras import models
    model_dir = 'models_One/model_' + str(iteration_idx)
...
def _evaluate_on_data_set_(thread_idx, return_dict, qsa_model, EPSILON, data_idx):
    ...
    first_shot = generate()
    model_chosen = sample_model_choice_from_prob(first_shot)
    first_performance = get_performance_of_encoded_model((X, y), model_chosen)
    max_accuracy_now = first_performance[1]
    for step in range(num_of_steps):
        ...
        reward_history.append(max(0, accuracy - max_accuracy_now))
...
def reinforcement_learning(num_iterations):
    #creating a network
    call(['rm', '-rf', 'models_One'])
    call(['mkdir', 'models_One'])
    process = Process(target=create_Q_network, args=(8, 29, 17, 4, 17, 5))
    process.start()
    process.join()
    for iteration_idx in range(num_iterations):
        #evaluate
        m, p, c, a, q = evaluate(iteration_idx, 32, 1028)
        #train
        process = Process(target=train_network, args=(iteration_idx, m, c, p, a, q))
        process.start()
        process.join()

reinforcement_learning(200)
