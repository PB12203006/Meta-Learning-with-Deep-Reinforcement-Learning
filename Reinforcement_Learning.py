...
def train_network(qsa_model, meta_data, model_choices, model_performances, actions, values):
...
def evaluate(qsa_model, loop_num, num_threads, sample_count):
    ...
    for _ in range(int(sample_count / num_threads)):
        ...
        for thread_idx in range(num_threads):
            mt = myThread(evaluate_on_data_set, (thread_idx, return_dict, qsa_model, 38, 17, 4, 17, 0.1,))
            jobs.append(mt)
        for thread in jobs:
            thread.start()
        for thread in jobs:
    ...
    np.save('loop' + str(loop_num), averaged_curve)
...
def evaluate_on_data_set(args):
    thread_idx, return_dict, qsa_model, METADATA_DIM, ACTION_DIM, PERFORMANCE_DIM, CHOICE_DIM, EPSILON = args
    np.random.seed(seed=thread_idx * int(time.time()))
    num_of_steps = 10
    #Generate Data set
    #Get metastatistics
...
def reinforcement_learning(num_iterations):
    qsa_model = create_Q_network(16, 38, 17, 4, 17, 10)
    qsa_model.save('model_%d' % 0)
    for iteration_idx in range(num_iterations):
        qsa_model._make_predict_function()
        m, p, c, a, q = evaluate(qsa_model, iteration_idx, 2, 4)
        train_network(iteratioin_idx, m, c, p, a, q)
        K.clear_session()
